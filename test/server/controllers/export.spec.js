...
describe(MODULE_ID, function() {
	...
	const DownloadsHeaders = Object.keys(EXPORT.downloads)
		.map(key => EXPORT.downloads[key])
		.join(',');
	const SavedItemsHeaders = Object.keys(EXPORT.saved_items)
		.map(key => EXPORT.saved_items[key])
		.join(',');
	...
	const downloadedItems = [
		{
			...
			content_id:
				'http://www.ft.com/thing/c7923fba-1d31-39fd-82f0-ba1822ef20d2',
		...
		},
		{
			...
			content_id:
				'http://www.ft.com/thing/b59dff10-3f7e-11e7-9d56-25f963e998b2',
	...
	const savedItems = [
		...
		},
		{
			...
			user_id: 'b2697f93-52d3-4d42-8409-bdf91b09e894',
			content_id:
				'http://www.ft.com/thing/dbe4928a-5bec-11e7-b553-e2df1b0c3220',
		...
		describe('downloads', function() {
			before(async function() {
				...
				req = httpMocks.createRequest({
					...
					headers: {
						'ft-real-url':
							'https://www.ft.com/syndication/export?type=downloads',
		...
		describe('saved items', function() {
			before(async function() {
				...
				req = httpMocks.createRequest({
					...
					headers: {
						'ft-real-url':
							'https://www.ft.com/syndication/export?type=saved_items',
				...
				expect(res.send).to.have.been.calledWith(
					Buffer.from(
						[SavedItemsHeaders]
							.concat(
								savedItems.map(item =>
									Object.keys(EXPORT.saved_items)
										.map(key => item[key])
										.join(',')
								)
							)
