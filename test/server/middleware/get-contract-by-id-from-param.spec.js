...
describe('Middleware: get-contract-by-id-from-param', () => {
	...
	beforeEach(() => {
		...
		next = spy();
		middleware = proxyquire('../../../server/middleware/get-contract-by-id-from-param', {'../lib/get-contract-by-id': () => getContractReturnValue});
	});
	...
	it('should call next with no arguments and set res.locals.contract if the contract returns successfully', done => {
		...
		getContractReturnValue = Promise.resolve({contractId: req.params.contract_id});
		middleware(req, res, err => {
			expect(err).to.equal(undefined);
			expect(res.locals).to.have.property('contract');
			expect(res.locals.contract).to.have.property('contractId').to.equal(req.params.contract_id);
			done()
		});
	});
	...
	it('should call next with the thrown error if getting the contract throws an error', done => {
		...
		middleware(req, res, err => {
			expect(err).to.have.property('message').to.equal('Wooopsy Daisy');
	...
	it('should call sendStatus 400 if there is no contract_id param', () => {
		...
		expect(res.send.lastCall.args[0]).to.have.property('message').to.equal('Missing path parameter: \'contract_id\'')
	});
