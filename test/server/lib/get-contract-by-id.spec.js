	...
	describe('success', function() {
		...
		it('returns contract data', async function() {
			...
			const expected = Object.assign(
				JSON.parse(JSON.stringify(contractResponse)),
				{
					content_allowed: 'Articles, Videos & Podcasts',
					contract_date: `${moment(contractResponse.start_date).format(
						'DD/MM/YY'
					)} - ${moment(contractResponse.end_date).format('DD/MM/YY')}`,
				}
			);

			expected.items[0] = expected.itemsMap['FT Article'] = expected.itemsMap[
				'article'
			] = Object.assign(
				JSON.parse(JSON.stringify(expected.itemsMap['article'])),
				{
					current_downloads: {
						day: 0,
						month: 0,
			...
			expected.items[1] = expected.itemsMap['Video'] = expected.itemsMap[
				'video'
			] = Object.assign(
				JSON.parse(JSON.stringify(expected.itemsMap['video'])),
				{
					current_downloads: {
						day: 0,
						month: 0,
			...
			expected.items[2] = expected.itemsMap['Podcast'] = expected.itemsMap[
				'podcast'
			] = Object.assign(
				JSON.parse(JSON.stringify(expected.itemsMap['podcast'])),
				{
					current_downloads: {
						day: 0,
						month: 0,
