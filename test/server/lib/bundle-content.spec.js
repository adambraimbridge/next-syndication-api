...
describe.skip(MODULE_ID, function() {
	...
	describe('With a transcript and captions', function() {
		...
		before(function(done) {
			...
			let content = (res.__content = require(path.resolve(
				`${FIXTURES_DIRECTORY}/${CONTENT_ID}.json`
			)));
			...
			let uri = url.parse(
				content.dataSource[content.dataSource.length - 1].binaryUrl
			);
				...
				.reply(
					...
					{},
					{
						'content-length': fs.readFileSync(
							path.resolve(`${FIXTURES_DIRECTORY}/video-small.mp4`)
						).length,
					}
				);
		...
		it(`should download the ${DOWNLOAD_ARCHIVE_EXTENSION} file`, function() {
			...
			expect(file.length)
				.to.be.above(0)
				.and.at.least(
					fs.readFileSync(path.resolve(`${FIXTURES_DIRECTORY}/video-small.mp4`))
						.length
				);
		});
		...
		it(`extracting the ${DOWNLOAD_ARCHIVE_EXTENSION} file should contain a video matching the downloaded resource`, function() {
			...
			const videoFile = extractedFiles.find(
				file => path.extname(file.path) === '.mp4'
			);
		...
		it("the video's file name should match the content.fileName", function() {
			expect(extractedFiles[0].path).to.equal(
				`${res.__content.fileName}.${res.__content.download.extension}`
			);
		});

		it(`extracting the ${DOWNLOAD_ARCHIVE_EXTENSION} file should contain a transcript in docx format`, function() {
			const transcriptFile = extractedFiles.find(
				file => path.extname(file.path) === '.docx'
			);

			expect(transcriptFile.data)
		...
		it("the transcript's file name should match the content.fileName", function() {
			...
			expect(transcriptFile.path).to.equal(
				`${res.__content.fileName}.${res.__content.transcriptExtension}`
			);
		});
	});
