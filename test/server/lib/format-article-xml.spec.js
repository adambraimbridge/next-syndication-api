...
const CHECK_NO_ELEMENTS = FORMAT_ARTICLE_CLEAN_ELEMENTS.concat(
	FORMAT_ARTICLE_STRIP_ELEMENTS
);
...
<p><em>Illustration by Christopher de Lorenzo</em></p>
	...
	it(`returns a single line with all ${CHECK_NO_ELEMENTS.map(
		el => `<${el} />`
	).join(', ')} elements removed and/or stripped`, function() {
		expect(
			underTest(xmlPre)
				.toString()
				.trim()
	...
	[
		...
		describe(`Testing content#${contentId}`, function() {
			const content = require(path.resolve(
				`${FIXTURES_DIRECTORY}/content/${contentId}.json`
			));
			...
			CHECK_NO_ELEMENTS.forEach(tagName => {
				it(`doc should not contain any <${tagName} /> elements`, function() {
					expect(doc.getElementsByTagName(tagName))
