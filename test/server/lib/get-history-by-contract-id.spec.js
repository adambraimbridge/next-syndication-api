...
describe(MODULE_ID, function() {
	...
	describe('default call', function() {
		...
		beforeEach(function() {
			...
			underTest = proxyquire('../../../server/lib/get-history-by-contract-id', {
				...
				'./get-all-existing-items-for-contract': sinon
					.stub()
					.resolves(itemsMap),
				'@noCallThru': true,
			});
		...
		it('db query', async function() {
			...
			expect(db.run).to.be.calledWith(
				`SELECT * FROM syndication.get_downloads_by_contract_id($text$${contract_id}$text$::text);`
			);
		});
	...
	describe("show only current user's items", function() {
		...
		it('db query', async function() {
			...
			expect(db.run).to.be.calledWith(
				`SELECT * FROM syndication.get_downloads_by_contract_id($text$${contract_id}$text$::text, $text$${user_id}$text$::text);`
			);
		});
	...
	describe('show only saved items', function() {
		...
		it('db query', async function() {
			...
			expect(db.run).to.be.calledWith(
				`SELECT * FROM syndication.get_saved_items_by_contract_id($text$${contract_id}$text$::text);`
			);
		});
