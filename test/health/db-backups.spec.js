...
describe(MODULE_ID, function() {
	...
	before(function() {
		...
		S3Stub = sinon.stub(__proto__, 'listObjectsV2').callsFake((params, cb) =>
			cb(null, {
				IsTruncated: false,
				Contents: [
					{
						Key: `production/syndication.${time.format(
	...
	describe('#tick', function() {
		it('calls S3.listObectsV2 to try and find the last back up file', async function() {
			...
			expect(S3Stub).to.have.been.calledWith({
				...
				MaxKeys: 5,
				Prefix: `${DB.BACKUP.bucket.directory}/${
					DB.BACKUP.schema
				}.${time.format(DB.BACKUP.date_format)}`,
			});
		});
