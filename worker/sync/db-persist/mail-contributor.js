...
const { CONTRIBUTOR_EMAIL } = require('config');
...
const transporter = nodemailer.createTransport(
	JSON.parse(JSON.stringify(CONTRIBUTOR_EMAIL.transport))
);

const HTML = Handlebars.compile(
	fs.readFileSync(
		path.resolve('./server/views/partial/email_contributor_body.html.hbs'),
		'utf8'
	),
	{ noEscape: true }
);
const TXT = Handlebars.compile(
	fs.readFileSync(
		path.resolve('./server/views/partial/email_contributor_body.txt.hbs'),
		'utf8'
	),
	{ noEscape: true }
);
...
module.exports = exports = async event => {
	if (
		event.syndication_state !== 'withContributorPayment' ||
		event.state !== 'started'
	) {
		return;
	}
	...
	const [contributor_payment] = await db.syndication.get_contributor_purchase([
		event.contract_id,
		event.content_id,
	]);
	...
	try {
		...
		event.displayDate = moment
			.tz(event.time, CONTRIBUTOR_EMAIL.timezone)
			.format(CONTRIBUTOR_EMAIL.date_display_format);
		...
		const data =
			contributor_payment && contributor_payment.contract_id !== null
				? contributor_payment
				: event;
