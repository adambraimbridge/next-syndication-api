...
module.exports = exports = async (req, res, next) => {
	...
	const {
		locals: { contract, flags },
	} = res;

	if (!Array.isArray(body)) {
		log.error(
			`${MODULE_ID} Expected \`req.body\` to be [object Array] and got \`${Object.prototype.toString.call(
				body
			)}\` instead`,
			{
	...
	const DISTINCT_ITEMS = Object.keys(
		body.reduce((acc, id) => {
			acc[id] = id;
	...
	log.info(
		`${MODULE_ID} => ${DISTINCT_ITEMS_LENGTH} distinct items found out of ${
			body.length
		} total items`
	);
	log.info(
		`${MODULE_ID} => Retrieved ${
			items.length
		}/${DISTINCT_ITEMS_LENGTH} distinct items in ${Date.now() - START}ms`
	);

	const existing = await getAllExistingItemsForContract(contract.contract_id);

	const response = items.map(src =>
		syndicate({
			contract,
