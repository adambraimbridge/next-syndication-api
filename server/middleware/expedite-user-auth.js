...
module.exports = exports = async (req, res, next) => {
	const {
		locals: { $DB: db, MAINTENANCE_MODE, flags, userUuid },
	} = res;
	...
	if (MAINTENANCE_MODE !== true) {
		try {
			...
			if (flags.syndicationMigrationComplete) {
				...
				const [mu] = await db.run(
					`SELECT * FROM syndication.get_migrated_user($text$${userUuid}$text$);`
				);
			...
			if (expedite === true) {
				...
				if (
					Date.now() - user.last_modified <
					SALESFORCE_REFRESH_CONTRACT_PERIOD
				) {
