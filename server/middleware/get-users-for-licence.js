...
const { ALS_API_KEY, BASE_URI_FT_API } = require('config');
...
module.exports = exports = async (req, res, next) => {
	const URI = `${BASE_URI_FT_API}/licence-seat-holders/${
	...
	try {
		...
		if (licenceRes.ok) {
			...
			if (Array.isArray(seatHolders) && seatHolders.length) {
				...
				log.info(
					`${MODULE_ID} => Found ${seatHolders.length} users for licence#${
						res.locals.licence.id
					}; contract#${res.locals.syndication_contract.id}`
				);
			} else {
				log.warn(
					`${MODULE_ID} => Found NO users for licence#${
						res.locals.licence.id
					}; contract#${res.locals.syndication_contract.id}`
				);
			}
