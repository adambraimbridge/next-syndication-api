...
(async () => {
	try {
		...
		log.debug(`${MODULE_ID} | ${Contracts.TableName} => `, {
			res: JSON.stringify(res, null, 4),
		});
	} catch (e) {
		log.error(`${MODULE_ID} | ${Contracts.TableName} => `, {
			error: e.stack,
		});
	}
	...
	try {
		...
		log.debug(`${MODULE_ID} | ${History.TableName} => `, {
			res: JSON.stringify(res, null, 4),
		});
	} catch (e) {
		log.error(`${MODULE_ID} | ${History.TableName} => `, {
			error: e.stack,
		});
	}
})();
